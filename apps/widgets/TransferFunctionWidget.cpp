...
namespace ospray {
  namespace tfn_widget {
    TransferFunctionWidget::Line::Line() :
      line({vec2f(0, 0),
	    vec2f(1, 1)}),
      color(0xffffffff)
    {
    ...
    void TransferFunctionWidget::Line::movePoint(const float &startX, const vec2f &end)
    {
	...
	*fnd = end;
    ...
    void TransferFunctionWidget::Line::removePoint(const float &x)
    {
      if (line.size() == 2)
	return;
    ...
    TransferFunctionWidget::TransferFunctionWidget(std::shared_ptr<sg::TransferFunction> tfn) :
      transferFcn(tfn),
      activeLine(3),
      tfcnSelection(JET),
      customizing(false),
    ...
    TransferFunctionWidget::TransferFunctionWidget(const TransferFunctionWidget &t) :
    ...
    TransferFunctionWidget& TransferFunctionWidget::operator=(const TransferFunctionWidget &t)
    {
      if (this == &t)
	return *this;
    ...
    void TransferFunctionWidget::load(const ospcommon::FileName &fileName)
    {
    ...
    void TransferFunctionWidget::save(const ospcommon::FileName &fileName) const
    {
      // For opacity we can store the associated data value and only have 1 line,
      // so just save it out directly
      tfn_reader::TransferFunction output(transferFunctions[tfcnSelection].name,
